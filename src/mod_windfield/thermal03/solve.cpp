...
void SolveFourthOrder::solve()
{
  ...
  // Ich interessiere mich zun�chst f�r die Extrempunkte der Funktion
  // (Nullstellen der Ableitung).
  //
  // N�heres dazu siehe Bronstein S. 131 und S.132 (alt).
  xi[0] = 0;
  xi[1] = 1;
  ...
  while (fSolved == false)
  {
         ...
         ))
    {
      ...
      // Bisektionsverfahren
      // Newton funktioniert hier nicht zuverl�ssig! Wenn man ihn an einer Stelle
      ...
      while (loopcnt < MAX_LOOP && fabs(xa-xb) > prec)
      {
        ...
        // Eigentlich darf das nicht passieren, denn hier kann nur pow(x, y) einen Fehler ausl�sen,
